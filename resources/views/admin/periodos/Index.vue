...
const periodosData = computed(() => page.props.periodos?.data || []);
const pagination = computed(
    () =>
        page.props.periodos?.pagination || {
            current_page: 1,
            last_page: 1,
            total: 0,
        },
...
function showUrl(id) {
    return route("admin.periodos-academicos.show", id);
}

function editUrl(id) {
    return route("admin.periodos-academicos.edit", id);
}

function togglePeriodo(periodo) {
    if (!window.confirm("¿Está seguro de cambiar el estado de este periodo?")) {
        return;
    }

    router.delete(route("admin.periodos-academicos.destroy", periodo.id), {
        preserveScroll: true,
        preserveState: true,
    });
}
